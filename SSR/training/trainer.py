...
class SSRTrainer(object):
    ...
    def set_params(self):
        ...
        self.lrate = float(eval(self.config["train"]["lrate"]))
    ...
    def step(
        ...
        sampled_data = self.sample_data(global_step, self.rays, self.H, self.W, no_batching=self.no_batching, mode="train")
        if self.enable_semantic:
        ...
        if global_step % self.config["logging"]["step_vis_train"]==0 and global_step > 0:
            ...
            if self.enable_semantic:
                ...
                tqdm.write(f"[Training Metric] Iter: {global_step} "
                       f"img_loss: {total_img_loss.item():.4f}, semantic_loss: {(total_sem_loss*wgt_sem_loss).item():.4f},"
                       f"psnr_coarse: {psnr_coarse.item():.4f}, psnr_fine: {psnr_fine.item():.4f},"
                       f"mIoU: {miou_train:.4f}, total_acc: {total_accuracy_train:.4f}, avg_acc: {class_average_accuracy_train:.4f}")
        ...
        if global_step%self.config["logging"]["step_log_print"]==0:
            tqdm.write(f"[TRAIN] Iter: {global_step} "
                       ...
                       f"rgb_total_loss: {total_img_loss.item():.4f}, rgb_coarse: {img_loss_coarse.item():.4f}, rgb_fine: {img_loss_fine.item():.4f}, "
                       f"sem_total_loss: {total_sem_loss.item():.4f}, weighted_sem_total_loss: {total_sem_loss.item()*wgt_sem_loss:.4f}, "
                       f"semantic_loss: {semantic_loss_coarse.item():.4f}, semantic_fine: {semantic_loss_fine.item():.4f}, "
                       f"PSNR_coarse: {psnr_coarse.item():.4f}, PSNR_fine: {psnr_fine.item():.4f}")
